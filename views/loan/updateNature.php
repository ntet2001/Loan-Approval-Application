<?php
    ...
    $queryId="SELECT nature FROM type_engagement WHERE id_type_engagement='$id'";
    ...
    if (!empty($_POST['nature'])) {
        $data=[];
        $data[0]=$_POST['nature'];
        $queryUpdate="UPDATE type_engagement SET nature='$data[0]' WHERE id_type_engagement='$id'";
...
<body>
    ...
    <div class="row formulaire">
            <div class="col-lg-6">
                <!--formulaire-->
                <form action="<?='updateNature.php?id='.$id?>" method="post">
                    <h1>Modify Nature Loan</h1><br>
                    <div class="form-group">
                        <?php
                            while (odbc_fetch_row($selectId)) {
                                $nature=odbc_result($selectId,'nature');
                            }
                        ?>
                        <label for="nature"><h5>Enter Nature of loan:</h5></label><br>
                        <input type="text" name="nature" id="nature" class="form-control" value="<?=htmlentities($nature)?>">
                    </div>
                    <?=$erreur?><br>
                    <a href="./indexNature.php" class="btn btn-primary">Back</a>
