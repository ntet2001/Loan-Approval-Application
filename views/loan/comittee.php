<?php
    ...
    WHERE statut=6 AND client.id_agence='$idAgence'";
        ...
        if (isset($_POST['envoyer'])) {
            if (!empty($_POST['opinion']) && !empty($_POST['decision'])) {
                ...
                if ($decision == 1) {
                    ...
                    $queryUpdateEngagement="UPDATE engagement SET statut=7, montant_acc='$montantacc' WHERE num_dossier='$array[0]'";
                    $updateEngagement=odbc_exec($connexion,$queryUpdateEngagement);
                    header('Location: ./comittee.php');
                }else{
                    $queryUpdateTraiter="UPDATE traiter SET date_fin='$datefin',opinion='$opinion' WHERE num_dossier='$array[0]' AND id_user='$idUser'";
                    $updateTraiter=odbc_exec($connexion,$queryUpdateTraiter);
                    $queryUpdateEngagement="UPDATE engagement SET statut=7, montant_acc=0 WHERE num_dossier='$array[0]'";
                    $updateEngagement=odbc_exec($connexion,$queryUpdateEngagement);
                    header('Location: ./comittee.php');
                }
            }else{
...
<body>
    <main class="container" style="margin-top: 90px;">
        <h1 style="text-align: center;margin-bottom:20px;"> Commitee Decision Making</h1>
            <div class="formulaire" style="margin-bottom: 20px;">
                <form action="./comittee.php" method="post">
                    <div class="row">
                        <!---info client-->
                        <div class="col-lg-6">
                    ...
                    <button type="submit" class="btn btn-success" name="envoyer">save</button>
                </form>
            </div>
    </main>
    <script defer>
