<?php
    ...
    $queryId="SELECT nom_typePret FROM typePret WHERE id_typePret='$id'";
    $selectId=odbc_exec($connexion,$queryId);

    //je fais l'update
    if (!empty($_POST['nomTypePret'])) {
        $data=[];
        $data[0]=$_POST['nomTypePret'];
        $queryUpdate="UPDATE typePret SET nom_typePret='$data[0]' WHERE id_typePret='$id'";
...
<body>
    ...
    <div class="row formulaire">
            <div class="col-lg-6">
                <!--formulaire-->
                <form action="<?='updateType.php?id='.$id?>" method="post">
                    <h1>Modify Type Loan</h1><br>
                    <div class="form-group">
                        <?php
                            while (odbc_fetch_row($selectId)) {
                                $nomTypePret=odbc_result($selectId,'nom_typePret');
                            }
                        ?>
                        <label for="nomTypePret"><h5>Enter type of loan:</h5></label><br>
                        <input type="text" name="nomTypePret" id="nomTypePret" class="form-control" value="<?=htmlentities($nomTypePret)?>">
                    </div>
                    <?=$erreur?><br>
                    <a href="./indexType.php" class="btn btn-primary">Back</a>
