<?php
    ...
    if (!empty($_POST['profil'])) {
        $profil=$_POST['profil'];
        $queryUpdate="UPDATE profil SET nom_profil='$profil' WHERE id_profil='$id'";
...
<body>
    ...
    <div class="row formulaire">
            <div class="col-lg-6">
                <!--formulaire-->
                <form action="<?='updateProfile.php?id='.$id?>" method="post">
                    <h1>Modified the Profile</h1><br>
                    <div class="form-group">
                        <label for="profil"><h3>Profile Name:</h3></label><br>
                        <!-- j'affiche le profil recuperer -->
                        <?php while(odbc_fetch_row($select)):?>
                            <?php
                                $nomprofil=odbc_result($select,'nom_profil');    
                            ?>
                            <input type="text" name="profil" id="profil" class="form-control" placeholder="Name" value="<?=htmlentities($nomprofil)?>">
                        <?php endwhile;?>
                        <!-- ferme la connexion -->
                        <?php finconnexion();?>
                    </div>
                    <a href="./indexProfile.php" class="btn btn-primary">Back</a>
