<?php
    ...
    if (!empty($_POST['nomPole']) && !empty($_POST['nomSection'])) {
        ...
        $queryInsert="INSERT INTO pole (nom_pole,id_section) VALUES('$data[0]','$data[1]')";
...
<body>
    ...
    <main class="container">
        ...
        <div class="row formulaire">
            <div class="col-lg-6">
                <form action="indexPole.php" method="post" autocomplete="off">
                    <div class="form-group">
                        <label for="nomPole"><h3>Pole Name</h3></label><br>
                        <input type="text" name="nomPole" id="nomPole" class="form-control" placeholder="ex:Name"><br>
                        ...
                        <select name="nomSection" id="nomSection" class="form-control">
                           <?php while(odbc_fetch_row($Resultatsection)):?>
                                ...
                                ?>
                                <option value="<?=$idSection?>"><?=$nomSection?></option>
                            <?php endwhile;?>
                        </select>
                    </div>
                    <?=$erreur?><br>
        ...
        <div class="row tableau" style="margin-top: 50px;">
            <div class="col">
                <caption><h4>ALL POLES</h4></caption>
                <table id="example" class="table table-hover table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        ?>
                        <?php foreach ($id_sections as $id_section):?>
                            <?php 
                                $queryPole="SELECT id_pole, pole.nom_pole,section.nom_section FROM pole
                                INNER JOIN section
                                ON pole.id_section=section.id_section
                                WHERE pole.id_section='$id_section'";
                                $ResultatPole=odbc_exec($connexion,$queryPole);    
                            ?>
                            <?php while(odbc_fetch_row( $ResultatPole)): ?>    
                                <?php
                                    $idPole=odbc_result($ResultatPole,'id_pole');
                                    $nomPole=odbc_result($ResultatPole,'nom_pole');    
                                    $section=odbc_result($ResultatPole,'nom_section');
                                ?>
                                <tr>
                                    <td><?=$idPole?></td>
                                    <td><?=$nomPole?></td>
                                    <td><?=$section?></td>
                                    <td>
                                        <a href="<?='./updatePole.php?id='.$idPole?>" class="btn btn-warning">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
                                                <path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/>
                                            </svg> Modify
                                        </a> 
                                        <a href="<?='./deletePole.php?id='.$idPole?>" class="btn btn-danger">
