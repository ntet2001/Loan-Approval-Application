<?php
   ...
   foreach ($idSections as $idSection) {
    ...
    $queryClient="SELECT id_client, num_client,nom_client,email_client,id_agence,telephone_client,prenom_client FROM client 
    WHERE id_client='$id'";
    ...
    if (!empty($_POST['numClient']) && !empty($_POST['nomClient']) && !empty($_POST['prenomClient']) && !empty($_POST['emailClient']) && !empty($_POST['telephoneClient']) && !empty($_POST['AgenceClient'])) {
        ...
        $queryInsert="UPDATE client SET num_client='$data[0]',nom_client='$data[1]',
        prenom_client='$data[2]',email_client='$data[3]',telephone_client='$data[4]',id_agence='$data[5]' WHERE id_client='$id'";
...
<body>
    <main class="container" style="margin-bottom:50px;margin-top:20px;">
    <div class="row formulaire">
            <div class="col-lg-6">
                <!--formulaire-->
                <form action="<?='updateClient.php?id='.$id?>" method="post" autocomplete="off">
                    ...
                    <?php
                        while (odbc_fetch_row($selectClient)) {
                            ...
                            $prenomClient=odbc_result($selectClient,'prenom_client');
                            $telephoneClient=odbc_result($selectClient,'telephone_client');
                            $agenceId=odbc_result($selectClient,'id_agence');
                        }
                    ?>
                    <div class="form-group"> 
                        <label for="numClient">Customer No:</label><br>
                        <input type="text" name="numClient" id="numClient" class="form-control" value="<?=htmlentities($numClient)?>">
                        <label for="nomClient">Custormer Name</label><br>
                        <input type="text" name="nomClient" id="nomClient" class="form-control" value="<?=htmlentities($nomClient)?>">
                        <label for="prenomClient">Last Name</label><br>
                        <input type="text" name="prenomClient" id="prenomClient" class="form-control" value="<?=htmlentities($prenomClient)?>"> 
                        <label for="emailClient">Customer Email</label><br>
                        <input type="email" name="emailClient" id="emailClient" class="form-control" value="<?=htmlentities($emailClient)?>">
                        <label for="telephoneClient">Custormer Phone</label><br>
                        <input type="tel" name="telephoneClient" id="telephoneClient" class="form-control" value="<?=htmlentities($telephoneClient)?>">
                        ...
                        <select name="AgenceClient" id="AgenceClient" class="form-control">
                            <?php foreach ($idPoles as $idpole):?>
                                ...
                                <?php while (odbc_fetch_row($selectAgence)):?>
                                    ...
                                    ?>
                                    <?php
                                        if ($idAgence==$agenceId) {
                                            $attribut="selected='selected'";
                                        }
                                    ?>
                                    <option value="<?=$idAgence?>" <?=$attribut?>><?=$nomAgence?></option>
                                <?php endwhile?>
                            <?php endforeach?>
                        </select>
                    </div>
                    <?=$erreur?><br>
                    <a href="./indexClient.php" class="btn btn-primary">Back</a>
